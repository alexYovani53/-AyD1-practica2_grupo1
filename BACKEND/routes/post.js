...
router.get('/verPost/:id_user', (req, res) => {
    ...
    connection.query(`select p.publicacion, p.imagen, p.id_usuario, date_format(p.fecha, '%d of %M of %Y at %H:%i') as fecha, u.nombre, u.foto, u.usuario 
    from publicacion as p, USUARIO as u where p.id_usuario =  ${id_user} and p.id_usuario = u.id_usuario
    union select p.publicacion, p.imagen, p.id_usuario, date_format(p.fecha, '%d of %M of %Y at %H:%i') as fecha, u.nombre, u.foto, u.usuario 
    from publicacion as p, USUARIO as u where p.id_usuario in ( select id_usuario2 from amistad where id_usuario1 = ${id_user})  and p.id_usuario = u.id_usuario
    order by fecha desc;`, (err, rows) => {
        try {
